...
DOCUMENT_PROCESSING_QUEUE_NAME = os.getenv(
...
def batch_start_processing(req: func.HttpRequest) -> func.HttpResponse:
    """
    Starts the processing of all documents received.

    Args:
        req (func.HttpRequest): The HTTP request object.

    Returns:
        func.HttpResponse: The HTTP response object indicating the status of the conversion process.
    """
    ...
    files_data = list(filter(lambda x: not x['embeddings_added'], files_data)) if req.params.get(
        'process_all') != 'true' else files_data
    files_data = list(map(lambda x: {'filename': x['filename']}, files_data))
    # Create the QueueClient object
    queue_client = QueueClient.from_connection_string(
        azure_blob_storage_client.connect_str, DOCUMENT_PROCESSING_QUEUE_NAME, message_encode_policy=BinaryBase64EncodePolicy())
