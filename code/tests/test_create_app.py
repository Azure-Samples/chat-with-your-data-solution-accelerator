    ...
    def test_conversation_azure_byod_returns_correct_response_when_not_streaming_without_data_keys(
        ...
        openai_client_mock.chat.completions.create.assert_called_once_with(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "system-message" + get_current_date_suffix(),
                }
            ]
            + self.body["messages"],
            temperature=0.5,
...
class TestGetFile:
    ...
    def test_get_file_client_authentication_error(self, mock_blob_client_class, client):
        ...
        mock_blob_client = MagicMock()
        mock_blob_client_class.return_value = mock_blob_client
        mock_blob_client.file_exists.side_effect = ClientAuthenticationError(
            "Auth failed"
        )
    ...
    def test_get_file_resource_not_found_error(self, mock_blob_client_class, client):
        ...
        mock_blob_client = MagicMock()
        mock_blob_client_class.return_value = mock_blob_client
        mock_blob_client.file_exists.side_effect = ResourceNotFoundError(
            "Resource not found"
        )
    ...
    def test_get_file_service_request_error(self, mock_blob_client_class, client):
        ...
        mock_blob_client = MagicMock()
        mock_blob_client_class.return_value = mock_blob_client
        mock_blob_client.file_exists.side_effect = ServiceRequestError(
            "Service unavailable"
        )
...
class TestGetMarkdownUrl:
    ...
    def test_formats_basic_markdown_link(self):
        ...
        result = get_markdown_url(
            "https://example.com/doc.pdf", "My Document", "sas123"
        )
    ...
    def test_url_encodes_special_characters(self):
        ...
        result = get_markdown_url(
            "https://storage.com/path with spaces/file.pdf", "Title", "sas"
        )
            ...
            "citations": [
                {
                    "title": "Doc",
                    "url": {
                        "source": "http://storage.com/file.pdf",
                        "id": "1",
                        "chunk": "0",
                    },
                    "content": "Content",
