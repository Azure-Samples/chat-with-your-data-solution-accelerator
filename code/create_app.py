...
def conversation_with_data(conversation: Request, env_helper: EnvHelper):
    ...
    if config.prompts.use_on_your_data_format:
        messages.append(
            {
                "role": "system",
                "content": config.prompts.answering_system_prompt + date_suffix,
            }
        )
...
def conversation_without_data(conversation: Request, env_helper: EnvHelper):
    ...
    messages = [
        {
            "role": "system",
            "content": env_helper.AZURE_OPENAI_SYSTEM_MESSAGE
            + get_current_date_suffix(),
        }
    ]
    ...
    def get_file(filename):
        ...
        try:
            ...
            if not re.match(r"^[^\x00-\x1f\x7f]+$", filename):
                logger.warning(
                    "Filename contains invalid control characters: %s", filename
                )
            ...
            logger.info(
                "File downloaded successfully: %s, size: %d bytes", filename, file_size
            )

            # For large files (>10MB), consider implementing streaming
            if file_size > 10 * 1024 * 1024:  # 10MB threshold
                logger.info(
                    "Large file detected: %s, size: %d bytes", filename, file_size
                )
            ...
            error_message = str(e)
            logger.exception(
                "Unexpected error downloading file %s: %s", filename, error_message
            )
