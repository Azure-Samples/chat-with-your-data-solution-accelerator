  ...
  QuestionInput: jest.fn((props) => {
    ...
    return (
      <div data-testid='questionInputPrompt' onClick={() => props.onSend("Let me know upcoming meeting scheduled")}>
        {props.placeholder}
        <div data-testid="microphone_btn" onClick={() => props.onMicrophoneClick()}>Microphone button</div>
...
describe('Chat Component', () => {
  ...
  test('clears chat when clear button is clicked', async () => {
    ...
    await waitFor(() => { expect(screen.queryByTestId("answer-response")).not.toBeInTheDocument() });
  });

  test('handles microphone click and starts speech recognition', async () => {
    // Mock the API response
    mockCallConversationApi.mockResolvedValueOnce({ ai_assistant_type: 'default' });
  ...
  test('handles stopping speech recognition when microphone is clicked again', async () => {
    ...
    await waitFor(() => { expect(screen.queryByText(/Listening.../i)).not.toBeInTheDocument() }); // Check if "Listening..." is removed
  });
              ...
              choices: [
                {
                  messages: [
                    {
                      role: 'tool',
                      content: JSON.stringify({ citations: ['Citation 1', 'Citation 2'] })
                    }
                  ]
                }
              ]
