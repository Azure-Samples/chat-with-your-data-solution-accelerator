...
class EnvHelper:
    ...
    This class provides methods to load and access environment variables
    used in the application.

    Attributes:
        AZURE_SEARCH_SERVICE (str): Azure Search service name.
        AZURE_SEARCH_INDEX (str): Azure Search index name.
        AZURE_SEARCH_KEY (str): Azure Search API key.
        ... (other environment variables)

    Methods:
        check_env(): Checks if all the required environment variables are set.
    """
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.AZURE_SEARCH_DIMENSIONS = os.getenv(
            'AZURE_SEARCH_DIMENSIONS', '1536')
        self.AZURE_SEARCH_FILENAME_COLUMN = os.getenv(
            'AZURE_SEARCH_FILENAME_COLUMN', 'filepath')
        self.AZURE_SEARCH_TITLE_COLUMN = os.getenv(
            'AZURE_SEARCH_TITLE_COLUMN', 'title')
        self.AZURE_SEARCH_URL_COLUMN = os.getenv(
            'AZURE_SEARCH_URL_COLUMN', 'url')
        self.AZURE_SEARCH_FIELDS_TAG = os.getenv(
        ...
        self.AZURE_BLOB_CONTAINER_NAME = os.getenv(
            'AZURE_BLOB_CONTAINER_NAME', '')
        # Azure Form Recognizer
        self.AZURE_FORM_RECOGNIZER_ENDPOINT = os.getenv(
            'AZURE_FORM_RECOGNIZER_ENDPOINT', '')
        self.AZURE_FORM_RECOGNIZER_KEY = os.getenv(
        ...
        self.AZURE_CONTENT_SAFETY_KEY = os.getenv(
        ...
        def check_env():
            ...
            This method iterates through the attributes of the `EnvHelper` class and checks if any of them have an empty value.
            If an attribute has an empty value, a warning message is logged.
            ...
            for attr, value in EnvHelper().__dict__.items():
                if value == '':
                    logging.warning(
                        f"{attr} is not set in the environment variables.")
