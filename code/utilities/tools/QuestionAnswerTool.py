...
class QuestionAnswerTool(AnsweringToolBase):
    """
    A tool for answering questions using a question-answering model.
    """
    ...
    def answer_question(self, question: str, chat_history: List[dict], **kwargs: dict):
        """
        Answers a given question using a question-answering model.

        Args:
            question (str): The question to be answered.
            chat_history (List[dict]): The chat history containing previous questions and answers.
            **kwargs (dict): Additional keyword arguments.

        Returns:
            Answer: The answer object containing the generated answer and relevant source documents.
        """
        config = ConfigHelper.get_active_config_or_default()
        answering_prompt = PromptTemplate(
            template=config.prompts.answering_prompt, input_variables=["question", "sources"])
        ...
        answer_generator = LLMChain(llm=llm_helper.get_llm(
        ), prompt=answering_prompt, verbose=self.verbose)
        sources_text = "\n\n".join(
