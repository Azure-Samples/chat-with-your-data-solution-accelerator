    ...
    def validate_config(config: dict):
        for document_processor in config.get("document_processors"):
            ...
            if (
                ...
                raise Exception(
                    f"Advanced image processing has not been enabled for document type {document_type}, but only {ADVANCED_IMAGE_PROCESSING_FILE_TYPES} file types are supported."
                )
