...
def get_site_id(site_hostname):
    if not site_hostname:
        raise ValueError(f"'site_hostname' can not be empty")
...
def get_page_headers(site_id):
    if not site_id:
        raise ValueError(f"'site_id' can not be empty")
    ...
    if response.status_code == 200:
        ...
        for page in pages_data:
            if '@odata.type' not in page or page['@odata.type'] != '#microsoft.graph.sitePage':
                continue
...
def get_site_page(site_id, page_header):
    if not page_header:
        raise ValueError(f"'page_header' can not be empty")
    ...
    if response.status_code == 200:
        ...
        for index, item in reversed(list(enumerate(page_data))):
            if '@odata.type' not in item or item['@odata.type'] != '#microsoft.graph.textWebPart' or 'innerHtml' not in item:
                continue
...
filter_by_name = lambda items, skip_names: list(filter(lambda x: x.name not in skip_names, items))
...
def scrap_sharepoint_data():
    ...
    pages_list = []
    for site_page in [get_site_page(site_id, page_header) for page_header in site_page_headers]:
        page_dict = {
