...
- **No exceptions.** If a genuine circular import surfaces (the only legitimate historical use case), the fix is **structural**: extract the shared type to a leaf module (e.g. [v2/src/shared/types.py](../../v2/src/shared/types.py) or a new `v2/src/shared/contracts/` package). This is a structural change and triggers Hard Rule #10 (ask the user first).
