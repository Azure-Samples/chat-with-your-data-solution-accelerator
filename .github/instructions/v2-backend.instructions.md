...
7. No direct DB clients in routers — call `databases.create(settings.database.db_type, ...)` from `v2/src/shared/providers/databases/`. Same rule for search (`shared/providers/search/`) and LLM (`shared/providers/llm/`).
