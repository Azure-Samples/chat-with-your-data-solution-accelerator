...
class LLMHelper:
    def __init__(
        ...
        self.temperature: float = (
            float(os.getenv("AZURE_OPENAI_TEMPERATURE", 0.7))
        ...
        self.document_loaders: BaseLoader = (
            WebBaseLoader if document_loaders is None else document_loaders
        )
        ...
        self.embeddings: OpenAIEmbeddings = (
            OpenAIEmbeddings(model=self.model, chunk_size=1)
        ...
        self.vector_store: VectorStore = (
            AzureSearch(
                azure_cognitive_search_name=self.vector_store_address,
        ...
        self.pdf_parser: AzureFormRecognizerClient = (
            AzureFormRecognizerClient() if pdf_parser is None else pdf_parser
        )
        self.blob_client: AzureBlobStorageClient = (
            AzureBlobStorageClient() if blob_client is None else blob_client
        )
    ...
    def convert_file_and_add_embeddings(
        self, source_url, filename, enable_translation=False
    ):
    ...
    def get_embeddings_model(self):
        OPENAI_EMBEDDINGS_ENGINE_DOC = os.getenv(
            "AZURE_OPENAI_EMBEDDING_MODEL",
            os.getenv("OPENAI_EMBEDDINGS_ENGINE_DOC", "text-embedding-ada-002"),
        )
        OPENAI_EMBEDDINGS_ENGINE_QUERY = os.getenv(
            "AZURE_OPENAI_EMBEDDING_MODEL",
            os.getenv("OPENAI_EMBEDDINGS_ENGINE_QUERY", "text-embedding-ada-002"),
        )
        return {
