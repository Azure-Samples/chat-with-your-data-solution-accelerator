...
// Retrieve the Search Name from the Search Endpoint which will be in the format "https://uniquename.search.windows.net/"
// It will end in a slash. Bicep forces us to have a default, so we use a default that we can manipulate in the same way
// to reduce another condition.  length(azureAISearchEndpoint) - 9) cuts the https:// and the trailing slash. We then
// take the first "part" of the split which will be '' if there is no value set. If its null we assume the user is creating
