...
This runbook deploys the current `v2/src/**` code to the existing `cwyd-cdb-v2` azd environment **without re-provisioning infrastructure**. All Azure resources are assumed already deployed in resource group `rg-cwyd-cdb-v2`.

---

## 1. Resource inventory (existing)

The `cwyd-cdb-v2` azd environment in [v2/.azure/cwyd-cdb-v2/.env](../.azure/cwyd-cdb-v2/.env) binds the three v2 application services to these Azure resources:

| Service | Resource type | Resource name | Endpoint |
|---|---|---|---|
| `backend` | Container App | `ca-backend-cwyd2bh3kb` | `https://ca-backend-cwyd2bh3kb.yellowrock-7315fe2a.eastus2.azurecontainerapps.io` |
| `frontend` | App Service | `app-frontend-cwyd2bh3kb` | `https://app-frontend-cwyd2bh3kb.azurewebsites.net` |
| `function` | Function App | `func-cwyd2bh3kb` | `https://func-cwyd2bh3kb.azurewebsites.net` |
...
|---|---|
| AI Foundry project | `proj-cwyd2bh3kb` (account `aisa-cwyd2bh3kb`) |
| Chat history (Cosmos) | `cosmos-cwydcdbv23ane6` |
| Vector index (Search) | `srch-cwydcdbv23ane6` |
| Document storage | `stcwydcdbv23ane6` (container `documents`, queue `doc-processing`) |
| Event Grid topic | `evgt-cwydcdbv23ane6` |
| Content Safety | `cs-cwyd2bh3kb` |
| Speech | `spch-cwyd2bh3kb` |
| User-Assigned Managed Identity | `id-cwyd2bh3kb` |
...
# Expect subscription ff9b5430-90ea-44c0-8a00-e488c1bf56f4 and tenant 52b39610-0746-4c25-a83d-d4f89fadedfe.
...
cd v2
azd env list
# Expect cwyd-cdb-v2 marked DEFAULT=true. If not, run:
#   azd env select cwyd-cdb-v2
...
Builds [v2/docker/Dockerfile.backend](../docker/Dockerfile.backend), pushes to the Bicep-deployed ACR, and updates `ca-backend-cwyd2bh3kb`. Verify the new revision took traffic:

```powershell
az containerapp revision list -g rg-cwyd-cdb-v2 -n ca-backend-cwyd2bh3kb -o table
...
```powershell
az functionapp function show -g rg-cwyd-cdb-v2 -n func-cwyd2bh3kb --function-name batch_push -o tsv --query name
az functionapp function show -g rg-cwyd-cdb-v2 -n func-cwyd2bh3kb --function-name add_url -o tsv --query name
```
...
```powershell
(Invoke-WebRequest -Uri "https://app-frontend-cwyd2bh3kb.azurewebsites.net" -UseBasicParsing).StatusCode
# Expect 200.
```
...
Invoke-RestMethod -Uri "https://ca-backend-cwyd2bh3kb.yellowrock-7315fe2a.eastus2.azurecontainerapps.io/api/conversation" `
...
Foundry portal: open project `proj-cwyd2bh3kb` → **Agents** pane → expect rows `cwyd` (tools: `search`) and `rai` (no tools).

DB-side verification (Cosmos):

```powershell
az cosmosdb sql query `
  -a cosmos-cwydcdbv23ane6 `
...
|---|---|---|---|
| 1 | Backend liveness | `Invoke-RestMethod https://ca-backend-cwyd2bh3kb.yellowrock-7315fe2a.eastus2.azurecontainerapps.io/api/health` | `200`, body status `pass` |
...
| 4 | Frontend shell | Browser → `https://app-frontend-cwyd2bh3kb.azurewebsites.net` | React shell loads, no console errors |
...
| 7 | Indexing — blob ingest | Drop a small PDF into Blob container `documents` on `stcwydcdbv23ane6` | Within ~60 s `GET /api/admin/documents` lists the source with `chunk_count > 0` |
| 8 | Indexing — citation | Ask FE about the uploaded doc | Response includes a `citation` event referencing the source |
| 9 | Function logs healthy | `az functionapp log tail -g rg-cwyd-cdb-v2 -n func-cwyd2bh3kb` | One `batch_start` + one `batch_push` per blob, no poison-queue entries |
...
| `azd deploy` reports `solutionName` minLength violation | Wrong azd env active (likely a stale or empty env) | `azd env select cwyd-cdb-v2` then re-run |
...
| `/api/health/ready` returns `fail` for `foundry` check | Foundry data-plane RBAC missing on `id-cwyd2bh3kb` | Verify role `Azure AI Developer` on `aisa-cwyd2bh3kb` for the UAMI |
| Indexing pipeline silent after blob upload | Event Grid subscription on `evgt-cwydcdbv23ane6` not routed to the function queue | `az eventgrid event-subscription list --source-resource-id <evgt-id>` and verify the `doc-processing` queue is the endpoint |
