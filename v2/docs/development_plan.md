...
| 6 | RAG Indexing Pipeline (Split Functions) | ⏭ next | `batch_start`, `batch_push`, `add_url`, `search_skill` blueprints land under `v2/src/functions/`; ingestion-only extensions land under `v2/src/functions/core/`. |
...
| #35e | 5 | Live-reload of `app.state.settings` after `PATCH /api/admin/config` so config tweaks take effect without container restart; effective-config `GET /api/admin/config/effective` showing the merged result of env defaults + DB overrides. Deferred from #35c (Hard Rule #12). | `v2/src/backend/{app,routers/admin}.py`, `v2/src/backend/core/settings.py` | Phase 5 audit | ⏳ partial (2026-05-07: **(a) live-reload channel landed.** Lifespan in `backend/app.py` calls `await database_client.get_runtime_config()` once at startup and stashes the result on `app.state.runtime_overrides` (None when nothing persisted). PATCH `/api/admin/config` route reassigns the same attribute to the just-merged `RuntimeConfig` after every successful upsert (atomic Python attribute write). New `get_runtime_overrides(request)` dependency + `RuntimeOverridesDep` type alias in `backend/dependencies.py` is the read seam for downstream consumers. 7 new tests: 3 dep-level (none-when-unset / state-instance-when-set / none-when-attr-missing), 2 lifespan (load-when-persisted / none-when-cold-start), 2 admin-route (writes-back-after-PATCH / does-not-touch-on-422). **(b) `GET /api/admin/config/effective` still open** -- T+5 will land the env+overrides merge endpoint on top of this seam.) |
| #35f | 5 | Audit log for admin config mutations: every successful `PATCH /api/admin/config` writes a row to a new `admin_audit` Cosmos type / Postgres table capturing `who`, `when`, `before`, `after`. Deferred from #35c. | `v2/src/backend/core/providers/databases/**`, `v2/src/backend/routers/admin.py` | Phase 5 audit | ☐ open |
...
| 35e | Live-reload + effective-config GET | ⏳ partial (a done 2026-05-07) | (a) `app.state.runtime_overrides` channel landed (lifespan seed + PATCH writeback + `get_runtime_overrides` dep); (b) `GET /api/admin/config/effective` still open. |
