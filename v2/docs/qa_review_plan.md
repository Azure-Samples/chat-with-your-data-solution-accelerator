...
- **Backend debt is structurally drained.** Per dev_plan §0 row 7 + `U-P7-AUDIT-3` (2026-06-02), every backend §0.1 row originating in Phases 1–7 is ✅ cleared except the 5 listed here (I2, I4, I5, I6) which are externally blocked (FE-owned, upstream OSS, `#39`-gated, or post-Phase-7 hardening) plus I1 which is the FE half of an admin merge whose backend surface is complete. No new actionable backend debt was opened during the 2026-06-04 QA review.
- **`#39` is ✅ cleared, but `#35g` still gates on it.** `#39` `requires_role("admin")` RBAC narrowing shipped 2026-05-06 (per §0 row 5). `#35g` (per-tenant overrides) waits on a separate, not-yet-scheduled slice: Easy Auth tenant-id claim propagation through the principal claims surface. This is a real architectural dependency, not a documentation artifact.
