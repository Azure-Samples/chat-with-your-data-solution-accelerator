...
Both modes share the same backend code path. Chat reads dispatch through `search_registry.registry.get(settings.search.index_store)(...)`; chat history dispatches through `chat_history.registry.get(settings.chat_history.backend)(...)`. The `index_store` and `backend` settings are deployment-time env vars derived from `databaseType` by [`post_provision.py`](../scripts/post_provision.py). No `if/elif` over `databaseType` exists in backend code (Hard Rule #4).

**Known limitation — ingestion under `postgresql` mode is non-functional today.** The [`batch_push`](../src/functions/batch_push/blueprint.py) and [`add_url`](../src/functions/add_url/blueprint.py) Functions blueprints currently construct `azure.search.documents.SearchClient` directly instead of dispatching through the search registry, so blob uploads and URL-ingest requests crash against an unreachable Azure Search endpoint in Postgres-mode deployments. Chat reads still work end-to-end (the pgvector read path is wired), but the index is read-only until the blueprints are refactored. Tracked as `B2-INGEST-PGVECTOR` in [development_plan.md](development_plan.md) §0.1; the fix lives in Phase B2-INGEST (3 sequential units that move both blueprints onto `search_registry` dispatch).
