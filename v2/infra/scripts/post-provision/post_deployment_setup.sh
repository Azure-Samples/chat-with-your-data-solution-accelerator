...
cleanup() {
    ...
    if [ -n "$RESTORE_PG_NAME" ]; then
        ...
        az postgres flexible-server update --resource-group "$RESOURCE_GROUP" \
            ...
            || echo "⚠ WARNING: Failed to disable public access. Please disable manually."
    fi
}
trap cleanup EXIT
...
if [ -n "$SERVER_FQDN" ]; then
    ...
    SEARCH_ENDPOINT=$(az search service list --resource-group "$RESOURCE_GROUP" \
        --query "[0].hostName" -o tsv 2>/dev/null || true)
    if [ -n "$SEARCH_ENDPOINT" ]; then
        export AZURE_AI_SEARCH_ENDPOINT="https://${SEARCH_ENDPOINT}"
        echo "✓ Discovered AI Search: ${SEARCH_ENDPOINT}"
    fi
fi
