...
> **⚠️ Important**: Environment variables are automatically generated when you run `azd provision`. You may need to manually configure some values as described in the steps below.
...
> **⚠️ Important**: If your deployment uses WAF (Web Application Firewall) or VNET-secured resources with private endpoints, you must enable public network access on all Azure services (Storage, OpenAI, Cognitive Services, Key Vault, PostgreSQL, etc.) to allow local development traffic.
...
> **⚠️ Important**: Make sure this port matches the proxy configuration in `vite.config.ts` (Step 9.3)
